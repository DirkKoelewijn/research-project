...
# Settings
interface=enp0s25	# Name of the network interface for the two devices (find with 'ifconfig')
replace_ip=127.0.0.1	# The IP in the PCAP to replace with the destination IP
router_ip=192.168.1.1	# The IP of the router
output_prefix="rw"	# Prefix for rewritten file names


# Echo with sudo to force permissions
sudo echo "Rewriting '$1' to be send" 
...
cmd="tcprewrite -i $1 -o $new_f -N $replace_ip:$other_ip --enet-dmac=$other_mac --enet-smac=$mac"
...
then
	...
	# Create run command
	cmd_run="sudo tcpreplay -i $interface $new_f"
